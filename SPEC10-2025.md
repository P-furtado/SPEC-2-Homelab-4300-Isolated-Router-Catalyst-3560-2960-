...
2) Connect the router LAN uplink to the DIST switch (routed link):
```
...
3) Connect the DIST switch to the access switch (trunk):
```
...
DIST-SW(config)# hostname DIST-SW
...
## Step 4 — VLANs and SVIs on the DIST (Inter-VLAN Routing)
...
ACCESS-SW(config-if)# description Uplink to DIST Gi0/2
...
ISR4331(config-if)# description Link to DIST-SW Gi0/1
...
**Note:** If DHCP requests from VLAN20 do not reach the router because of your physical/routed setup, you may need `ip helper-address` on the DIST SVI for VLAN20 pointing to 192.168.30.1. Only configure helper if necessary in your environment.
...
```
DIST-SW# show ip ssh
DIST-SW# show running-config | section line vty
```
- Fix example:
```
DIST-SW(config)# ip domain-name homelab.local
DIST-SW(config)# crypto key generate rsa modulus 1024
DIST-SW(config)# ip ssh version 2
DIST-SW(config)# username admin privilege 15 secret cisco123
DIST-SW(config)# line vty 0 4
DIST-SW(config-line)# login local
...
```
DIST-SW(config)# line console 0
DIST-SW(config-line)# exec-timeout 15 0
DIST-SW# write memory
```
...
**DIST (3560) - example final running-config:**
```
! --- DIST-SW Running Configuration ---
hostname DIST-SW
enable secret class
...
interface GigabitEthernet0/1
 switchport mode trunk
 switchport trunk allowed vlan 10,20
 description Uplink to DIST-SW Gi0/2
 no shutdown
!
interface GigabitEthernet0/2
...
interface GigabitEthernet0/0/1
 ...
 description Link to DIST-SW Gi0/1
 no shutdown
!
